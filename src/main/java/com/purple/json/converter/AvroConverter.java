...
public class AvroConverter {
	...
	public String convert(final String json) throws IOException {
		...
		finalSchema.put("namespace", "com.purplecube.test");
	...
	private ArrayNode getFields(final JsonNode jsonNode) {
		...
		while (elements.hasNext()) {
			...
			case NUMBER:
				...
				numberNode.put(NAME, map.getKey());
				numberNode.putArray(TYPE).add(NULL).add((nextNode.isLong() ? "long" : "double"));
				fields.add(numberNode);
				break;

			case STRING:
				ObjectNode stringNode = mapper.createObjectNode();
				stringNode.put(NAME, map.getKey());
				stringNode.putArray(TYPE).add(NULL).add(STRING).add("double");
				fields.add(stringNode);
				break;
			...
			case ARRAY:
				...
				if (element.getNodeType() == JsonNodeType.NUMBER) {
					objectNode.set(TYPE, mapper.createObjectNode().put(TYPE, ARRAY).put(ITEMS,
							(nextNode.isLong() ? "long" : "double")));
			...
			case OBJECT:
				...
				node.set(TYPE, mapper.createObjectNode().put(TYPE, RECORD).put(NAME, generateRandomNumber(map))
						.set(FIELDS, getFields(nextNode)));
				fields.add(node);
				break;
		...
		System.out.println("AvroConverter.getFields() fields" + fields);
		return fields;
	}
